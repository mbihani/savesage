...
def configure_tracing(config: TracingConfig, mlflow_module: Any = None) -> None:
    ...
    if config.tracking_uri == "databricks" and config.databricks_profile:
        if "DATABRICKS_CONFIG_PROFILE" not in os.environ:
            # Only set the profile when ~/.databrickscfg exists (local dev).
            # In the Databricks Apps runtime the file is absent and the bound
            # experiment resource supplies auth — setting a non-existent
            # profile breaks the SDK's credential chain for tracing calls.
            ...
            if os.path.isfile(cfg_path):
                ...
                _LOGGER.info(
                    "tracing: skipping DATABRICKS_CONFIG_PROFILE"
                    " (no ~/.databrickscfg — Apps runtime, auth from bound resource)",
                )
...
class MLflowTraceSink(TraceSink):
    ...
    def _flush(self, ops: list[SpanOp], request_id: str) -> None:
        _LOGGER.info("tracing flush: %d spans for %s", len(ops), request_id)
