...
- `hdfc/score_lib.py:95-100,200-220`: canonical `norm_key`, numeric tolerance,
  ...
  these behaviors uniformly across all banks. This differs from the ICICI/SBI
  punctuation-preserving `text` + `lenient_hit` rule and is marginally more lenient
  for punctuation-only card-name differences, so it could only over-report agreement.
  Real card names in the evaluated corpus contain no punctuation; the deviation is
  deliberately accepted rather than adding bank-specific comparison paths.
